...
vr::EVRInitError OpenTrackDeviceProvider::Init(vr::IVRDriverContext* pDriverContext) {
    VR_INIT_SERVER_DRIVER_CONTEXT(pDriverContext);

    opentrack_device_ = std::make_unique<OpenTrackDeviceDriver>();

    if (!vr::VRServerDriverHost()->TrackedDeviceAdded(opentrack_device_->GetSerialNumber().c_str(), vr::TrackedDeviceClass_HMD, opentrack_device_.get())) {
